...
abstract class Order_Document_Methods extends Order_Document {
	...
	// public function get_pan( $pan_type = 'billing' ) {
	// 	$pan = '';
	// 	if ( ! empty( $order = $this->is_refund( $this->order ) ? $this->get_refund_parent( $this->order ) : $this->order ) ) {
	// 		$getter = "get_{$pan_type}_pan";
	// 		$pan  = is_callable( array( $order, $getter ) ) ? call_user_func( array( $order, $getter ) ) : $pan;
	// 	}
	// 	return $pan;
	// }

	// public function get_billing_pan() {
	// 	$phone = $this->get_pan( 'billing' );

	// 	return apply_filters( "wpo_wcpdf_billing_pan", $pan, $this );
	// }
	// public function billing_pan() {
	// 	echo $this->get_billing_pan();
	// }
	public function get_billing_aadhar() {
		global $wpdb;
		$order_date = $wpdb->get_row( "SELECT * FROM `wp_postmeta` WHERE post_id = ". $this->order_id ." AND meta_key like '%billing_aadhaar%'" );
		echo $order_date->meta_value;
		
	}
	public function billing_aadhar() { 
		echo $this->get_billing_aadhar();
	}
	public function get_billing_pan() {
		
		global $wpdb;
		$order_date = $wpdb->get_row( "SELECT * FROM `wp_postmeta` WHERE post_id = ". $this->order_id ." AND meta_key like '%billing_pan%'" );
		echo $order_date->meta_value ;
	...
	public function get_billing_payment() {
		
		global $wpdb;
		$order_date = $wpdb->get_row( "SELECT * FROM `wp_postmeta` WHERE post_id = ". $this->order_id ." AND meta_key like '%transaction_id%'" );
		echo $order_date->meta_value ;
		
		
	}
	public function billing_payment() {
		echo $this->get_billing_payment();
	}
	public function get_billing_charity() {
		
		global $wpdb;
		$order_date = $wpdb->get_row( "SELECT * FROM `wp_postmeta` WHERE post_id = ". $this->order_id ." AND meta_key like '%billing_type_of_charity%'" );
		echo $order_date->meta_value ;
		
		
	}
	public function billing_charity() {
		echo $this->get_billing_charity();
	}
	public function get_billing_amount() {
		
		global $wpdb;
		$order_date = $wpdb->get_row( "SELECT * FROM `wp_postmeta` WHERE post_id = ". $this->order_id ." AND meta_key like '%billing_amount%'" );
		echo $order_date->meta_value ;
		
		
	}
	public function billing_amount() {
		echo $this->get_billing_amount();
	}
