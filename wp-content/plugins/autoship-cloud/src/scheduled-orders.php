  ...
  if ( autoship_enable_wc_tax_settings_for_prices_display() ){
    ...
    */
    foreach ( $discounts as $code => $coupon ) {
      ...
      if ( 'ReduceSubtotalByPercentage' == $coupon['discountType'] ){
        ...
        // Get the Product ID
        $product_id = $coupon['product']['externalId'];
 ...
 * NOTE Site Settings should probably be stored in the db
 * Since they don't change often.  Could add expiration and refresh on expire.
 */
