...
function autoship_woocommerce_rest_fix_data($data) {
  ...
  $fixed_data = autoship_woocommerce_rest_fix_dates($fixed_data);
  return $fixed_data;
}
...
/**
 * Converts order date fields into ISO 8601 date formats for QPilot
 * TODO: legacy code needs review for refactor and see if still needed.
 *
 * @param array $data The Rest Response Order Data.
 * @return array The adjusted Data with new Date Fields.
 */
function autoship_woocommerce_rest_fix_dates( $data ) {
  $fixed_data = $data;
  ...
  foreach ($fixed_data as $key => $value) {
    if (is_array($value)) {
      $fixed_data[$key] = autoship_woocommerce_rest_fix_dates($value);
      continue;
    }
    if (in_array($key, $date_field_names, true)) {
      // This is a date field
      if (is_string($value)) {
        $time = strtotime($value);
        if ($time < 0) {
            // This is an invalid timestamp, so set it to null
            $fixed_data[$key] = null;
            continue;
        }
        // Re-format the timestamp
        $fixed_data[$key] = date('c', $time);
      } else {
        $fixed_data[$key] = null;
      }
    }
...
function autoship_woocommerce_rest_insert_shop_object_add_note( $order, $request, $creating ) {
  ...
  if ( !empty( $order_note ) ) {
    ...
    delete_post_meta( $order->get_id() , '_add_order_note' );
  }
}
