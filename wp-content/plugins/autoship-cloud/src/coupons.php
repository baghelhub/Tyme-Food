 ...
 * are calculated correctly.
 *
 * @param array $new_coupons The new coupons created from Request
 * @param array $coupons The raw coupon metadata from the request.
 * @param  WP_REST_Request $request The current Request data.
 * @return array The updated/filtered coupons.
 */
function autoship_qpilot_orders_update_via_rest_convert_shipping_to_coupons( $new_coupons, $coupons, $request ) {

  // Let's check for shipping coupons. While there is no corresponding type in WC we'll use a fixed cart at $0 to simply show merchants a shipping coupon was applied.
  foreach ( $coupons as $key => $coupon ) {

    $type = autoship_get_coupon_type_by_code( $coupon['discountType'], true );

    // Supported types return either string or NULL not false.
    if ( !$type && !is_null( $type ) )
    continue;

    // Empty means shipping since those return NULL.
    if ( is_null( $type ) ){
      $new_coupons['Coupon: ' . $coupon['code']] = autoship_generate_virtual_wc_coupon( array(
          'code'          => 'Coupon: ' . $coupon['code'],
          'description'   => sprintf( __( 'QPilot Dynamic Coupon generated from Coupon ID #%d.', 'autoship' ), $coupon['id'] ),
...
add_filter( 'autoship_rest_generated_virtual_wc_coupons', 'autoship_qpilot_orders_update_via_rest_convert_shipping_to_coupons', 10, 3 );
