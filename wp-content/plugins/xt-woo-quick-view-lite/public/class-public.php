...
class XT_Woo_Quick_View_Public
{
    ...
    public function enqueue_vendors()
    {
        ...
        if ( class_exists( 'WC_Product_Addons_Display' ) && is_a( $post, 'WP_Post' ) && has_shortcode( $post->post_content, 'xt_wooqv_trigger' ) ) {
            wp_enqueue_style(
                'woocommerce-addons-css',
                WC_PRODUCT_ADDONS_PLUGIN_URL . '/assets/css/frontend.css',
                array( 'dashicons' ),
    ...
    public function add_quick_view_trigger( $link, $product )
    {
        ...
        $quickViewButton = $this->render_trigger_button(
            $product_id,
            'a',
            false,
            true
