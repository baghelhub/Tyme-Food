...
$capabilities_goal_label = fpframework()->_('FPF_GOAL');
...
foreach ($templates as $template_key => $template)
{
    ...
    $capabilities = [
        ...
        'third_party_dependencies' => [
            'value' => $template->fields->external_plugins,
            'errors' => []
        ],
        'license_error' => [
            'value' => ''
    ...
    if (is_array($external_plugins) && count($external_plugins))
    {
        foreach ($external_plugins as $key => $external_plugin)
        {
            ...
            if (!\FPFramework\Helpers\WPHelper::isPluginInstalled($external_plugin->slug))
            {
                $icon = 'install';
            ...
            else if (!\is_plugin_active($external_plugin->slug))
            {
                $icon = 'activate';
            ...
            {
                $third_party_plugin_installed_version = \FPFramework\Helpers\WPHelper::getThirdPartyPluginVersion($external_plugin->slug);
    ...
    if ($plugin_license_type === 'lite' && $template->is_pro)
    {
        $errors = [
            'pro' => [
                ...
                'data_attributes' => 'data-fpf-modal-item="' . esc_attr($template->title) . '" data-fpf-modal="#fpfUpgradeToPro"',
