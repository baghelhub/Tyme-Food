...
<div class="fpf-field-item fpf-control-group-field fpf-searchdropdown-wrapper<?php esc_attr_e($is_multiple . $control_inner_class); ?>"
	...
	<?php if (count($items)): ?>
		...
		{
			$value = (array) $value;
			$index = $selected_items ? strval(array_search($value['id'], array_column($selected_items, 'id'))) : '';
			...
			if (is_object($value['title']))
			{
				...
				{
					$is_selected = strval(array_search($_key, array_column($selected_items, 'id')));;
					...
					if ($exclude_rules_pro && in_array($_key, $exclude_rules))
					{
						...
						$extra_atts = ' data-fpf-modal-item="' . esc_attr($_value . ' ' . fpframework()->_('FPF_CONDITION')) . '" data-fpf-modal="#fpfUpgradeToPro"';
					}
