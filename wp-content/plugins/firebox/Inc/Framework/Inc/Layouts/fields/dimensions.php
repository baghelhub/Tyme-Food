...
<ul class="fpf-dimensions-control<?php echo $this->data->get('class') ? esc_attr(' ' . implode(' ', $this->data->get('class'))) : ''; echo ($showLink) ? ' with-link' : ''; echo ($linked) ? ' is-linked' : ''; ?>">
	...
	foreach ($this->data->get('labels', []) as $key => $dimension_label)
	{
		...
		<li class="fpf-dimensions-control-item">
			<input type="number"<?php echo wp_kses_data($this->data->get('number_atts') . $this->data->get('required_attribute') . $this->data->get('extra_atts')); ?> id="fpf-control-input-item_<?php echo esc_attr($this->data->get('name_key') . '_' . $key); ?>" class="fpf-field-item fpf-control-input-item dimensions<?php echo esc_attr($this->data->get('input_class')); ?>" placeholder="<?php echo esc_attr($this->data->get('placeholder', '')); ?>" name="<?php echo esc_attr($this->data->get('name') . '[' . esc_attr($key)) . ']'; ?>" value="<?php echo esc_attr($dimension_value); ?>" />
