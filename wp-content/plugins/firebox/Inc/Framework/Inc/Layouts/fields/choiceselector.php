...
<div class="grid-x grid-margin-x grid-margin-y fpf-choice-selector-field<?php echo esc_attr($this->data->get('input_class')); ?>">
    ...
    foreach ($choices as $key => $_value)
    {
        ...
        ?>
        <div class="cell<?php echo esc_attr($this->data->get('choice_item_class')); ?> choice <?php echo $mode . ($pro ? ' pro fpf-modal-opener' : ''); ?>"<?php echo $pro ? ' data-fpf-modal="#fpfUpgradeToPro" data-fpf-modal-item="' . esc_attr(fpframework()->_($label)) . '"' : ''; ?>>
            <div class="inner">
                ...
                <input type="radio"<?php echo $this->data->get('required_attribute'); ?> id="fpf-control-input-item_<?php echo esc_attr($id); ?>" name="<?php echo esc_attr($this->data->get('name')); ?>" value="<?php echo esc_attr($key); ?>"<?php echo ($this->data->get('value') == $key) ? ' checked="checked"' : ''; ?> />
